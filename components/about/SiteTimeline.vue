<template>
  <v-timeline dense class="timeline-widget">
      <v-timeline-item
        v-for="item in items"
        :key="item.date"
        fill-dot
...
.timeline-widget h2, .timeline-widget p {
  font-weight:  400;
}
...
.mobile .timeline-widget h2 {
  font-size:  1.7rem;
}

.mobile .timeline-widget h3 {
  font-size:  1.3rem;
}

.timeline-date-headline {
  text-transform: uppercase;
  color: #666666;
}

.mobile .timeline-date-headline {
  font-size:  0.9rem;
}
</style>
